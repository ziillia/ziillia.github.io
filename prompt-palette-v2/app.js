(function(){
  ...
  function render(){
    ...
    const shooting=control(t('表情の指定','Expression controls'),chips('expression','expressions')+(state.variation==='keep'?'<p class="description">'+t('KEEPでは表情・視線の指定を適用せず、参照画像の顔を完全に固定します。','KEEP ignores expression and gaze choices and locks the reference face completely.')+'</p>':''))+
  ...
  function update(){
    ...
    $('summary-shooting').textContent=[state.variation==='keep'?t('顔を固定','Face locked'):name('expressions',state.expression),...state.sceneIds.map(id=>name('scenes',id)),...state.poseIds.map(id=>name('poses',id))].filter(Boolean).join(' · ');
  ...
  document.addEventListener('click',async event=>{
    ...
    else if(action==='export')download('prompt-palette-v23.json',JSON.stringify({format:'prompt-palette',state,collapsed:fold},null,2));
