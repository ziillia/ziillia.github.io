(function(){
  ...
  function update(){
    ...
    $('summary-finish').textContent=`${name('identities',state.identity)} · ${name('looks',state.look)} · ${state.variation.toUpperCase()} · ${state.layout} ${t('枚','shots')}`;
  ...
  document.addEventListener('click',async event=>{
    ...
    else if(action==='export')download('prompt-palette-v26.json',JSON.stringify({format:'prompt-palette',state,collapsed:fold},null,2));
