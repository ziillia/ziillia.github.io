(function(){
  ...
  function render(){
    ...
    const finish=control(t('人物の扱い','Subject identity'),chips('identity','identities','segmented'))+
      control(t('写真のトーン','Photographic tone'),chips('look','looks','segmented'))+
  ...
  function update(){
    ...
    $('compositionDescription').textContent=E.textOf(E.find(state,'compositions',state.composition),state.language);
    $('summary-finish').textContent=`${name('identities',state.identity)} · ${name('looks',state.look)} · ${state.variation.toUpperCase()} · ${state.layout} ${t('枚','shots')} · ${name('compositions',state.composition)}`;
    $('summary-shooting').textContent=[state.variation==='keep'?t('表情指定なし','Expression omitted'):name('expressions',state.expression),...state.sceneIds.map(id=>name('scenes',id)),...state.poseIds.map(id=>name('poses',id))].filter(Boolean).join(' · ');
  ...
  document.addEventListener('click',async event=>{
    ...
    if(button.dataset.path){
      ...
      state.take=0;save();if(['cameraMode','variation'].includes(path))render();else update();return;
    }
    ...
    else if(action==='export')download('prompt-palette-v28.json',JSON.stringify({format:'prompt-palette',state,collapsed:fold},null,2));
