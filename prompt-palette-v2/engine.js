...
(function(root){
  ...
  const defaults=()=>({schemaVersion:17,language:'jp',identity:'reference',look:'neutral',variation:'balanced',layout:'4',composition:'follow',sceneIds:[],poseIds:[],expression:'auto',cameraMode:'planned',distance:'auto',angle:'auto',lighting:'auto',body:{mass:'reference',regions:[],vascularity:'reference',bust:false,customIds:[]},outfitId:'',coverageId:'',take:0,custom:{scenes:[],poses:[],outfits:[],bodies:[]}});
  ...
  function normalize(raw){
    const d=defaults();
    const hadComposition=typeof raw?.composition==='string';
    const s={...d,...clone(raw||{}),schemaVersion:17};
    s.language=['jp','en'].includes(s.language)?s.language:'jp';
    for(const [key,group] of [['identity','identities'],['look','looks'],['variation','variations'],['composition','compositions'],['expression','expressions'],['cameraMode','cameraModes'],['distance','distances'],['angle','angles'],['lighting','lighting']])s[key]=valid(group,s[key],d[key]);
    if(!hadComposition&&raw?.cameraMode==='free-distinct')s.composition='ai-distinct';
